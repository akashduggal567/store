...
class expansionTile extends StatelessWidget {
  ...
  int index;
  Category categoryDetails;
  expansionTile({this.model, this.index, this.categoryDetails});
    ...
    model.subChildCategories.forEach((subChildCategory) {
      if (subChildCategory.parentCategoryIds.contains(subCategory.id)) {
        children.add(
          InkWell(
            onTap: () {
              model.navigateToProductListView(
                  [categoryDetails.id, subCategory.id, subChildCategory.id],
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Container(
        child: !checkIfSubChildExist(model.subCategories[index].id, model.subChildCategories)
            ? ListTile(
              ...
              onTap: (){
                model.navigateToProductListView(
                    [categoryDetails.id, model.subCategories[index].id],
                    categoryDetails.name,
                    model.subCategories[index].name);
              },
              )
...
bool checkIfSubChildExist(parentId, subChildArray) {

  for (var index = 0 ; index < subChildArray.length; index ++){
    if (subChildArray[index].parentCategoryIds.contains(parentId)) {
      return true;
    }
  }
  return false;
