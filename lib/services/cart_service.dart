...
class CartService with ReactiveServiceMixin {
  CartService() {
    listenToReactiveValues(
        [_cartItems, _totalAmount, _totalDiscountAmount, _totalAmountPayable, _totalItemCount]);
  }
...
//    } else {
  ...
  void fetchUserCart() async {
   ...
   void serialized = a.map((e) => _cartItems.add(serializeForCart(e))).toList();
   calcualteBillattributes();
   List<int> mrp =  _cartItems.map((item){
     return item.cartQuantity.toInt();
   }).toList();
   print(mrp);
   _totalItemCount = mrp.length == 0 ? RxValue(initial: 0) :  RxValue(initial: mrp.reduce((a, b) => a + b));
//   _totalItemCount = mrp.fold(RxValue(initial: 0), (previousValue, element) => previousValue+element);
  ...
  Product serializeForCart(DetailedCartItem element) {
    ...
    Map<String, dynamic> result = {...element.productId.toJson(), 'cart_quantity': element.cartQuantity};
